...
class StorageHandler:
    ...
    def get_instance(
        ...
        Create the first instance using the supplied path and initial data.
        Ignore new arguments when an instance already exists.
        """
        ...
        with cls._instance_lock:
            if cls._instance is None:
                if path is None or initial_data is None:
                    raise RuntimeError(
                        "StorageHandler must be initialized "
                        "with path and initial data."
                    )
